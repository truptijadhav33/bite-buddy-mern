...
export default function DashboardLayout({ title, menuItems }) {
  // Use persistent storage for sidebar state - a "pro" touch
  const [collapsed, setCollapsed] = useLocalStorage("sidebar-collapsed", false);

  const dispatch = useDispatch();
  const navigate = useNavigate();

  // Use our custom hook for cleaner auth access
  const { user } = useAuth();
  ...
  return (
    <div className="flex min-h-screen bg-[#0a0a0a] text-white">
      <aside
        className={`flex flex-col border-r border-white/10 bg-[#0f0f0f] transition-all duration-500 ease-in-out sticky top-0 h-screen ${collapsed ? "w-20" : "w-72"
          }`}
      >
        <div
          onClick={() => setCollapsed(!collapsed)}
          className={`h-20 flex items-center border-b border-white/10 px-6 cursor-pointer transition-all hover:bg-white/5 group ${collapsed ? "justify-center" : "justify-between"
            }`}
        >
          {collapsed ? (
        ...
        <nav className="flex-1 px-4 py-8 space-y-2 overflow-y-auto custom-scrollbar">
          {menuItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all group relative ${isActive
            ...
            >
              {({ isActive }) => (
                <>
                  <span
                    className={`text-xl transition-transform duration-300 group-hover:scale-110 ${isActive ? "drop-shadow-[0_0_5px_rgba(234,193,87,0.5)]" : ""
                      }`}
                  >
                    {item.icon}
                  </span>
        ...
        <div className="p-4 border-t border-white/10 bg-black/20">
          <button
            ...
            className={`flex items-center gap-4 w-full px-4 py-4 rounded-xl transition-all text-muted-foreground hover:bg-red-500/10 hover:text-red-400 group ${collapsed ? "justify-center" : ""
              }`}
          >
