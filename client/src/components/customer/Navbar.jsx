    ...
    <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] flex justify-center w-full px-4">
      ...
      <div className="bg-white/4 backdrop-blur-lg border border-white/10 rounded-full px-2 py-2 flex items-center shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative">
        <div className="flex items-center">
          {/* Public Links - Everyone can see these */}
          <NavLink
            to="/"
            icon={<Home size={20} />}
...
function NavLink({ to, icon, active, label }) {
  return (
    <Link
      to={to}
      className={`relative group flex flex-col items-center justify-center w-16 h-14 transition-all duration-300 ${
        active ? "text-primary" : "hover:text-[#ffffff]"
      }`}
    >
      {/* Active Glow Effect */}
      {active && (
      ...
      <span
        className={`text-[10px] mt-1 font-black uppercase tracking-widest transition-opacity duration-300 ${
...
function DropdownItem({ to, icon, label, color = "text-white/70" }) {
  return (
    <Link
      to={to}
